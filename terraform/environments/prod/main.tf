...
# STG and DEV VNets — for cross-environment peering
data "azurerm_virtual_network" "stg" {
  name                = var.stg_vnet_name
  resource_group_name = var.stg_resource_group_name
}
...
# Storage blob DNS zone already exists in the hub RG — only add a VNet link for prd
...
resource "azurerm_virtual_network_peering" "prd_to_stg" {
  name                      = "peer-prd-to-stg"
  resource_group_name       = azurerm_resource_group.network.name
  virtual_network_name      = azurerm_virtual_network.vnet.name
  remote_virtual_network_id = data.azurerm_virtual_network.stg.id
  allow_forwarded_traffic   = true
  use_remote_gateways       = false
}

resource "azurerm_virtual_network_peering" "stg_to_prd" {
  name                      = "peer-stg-to-prd"
  resource_group_name       = var.stg_resource_group_name
  virtual_network_name      = var.stg_vnet_name
  remote_virtual_network_id = azurerm_virtual_network.vnet.id
  allow_forwarded_traffic   = true
  use_remote_gateways       = false
}
...
resource "azurerm_virtual_network_peering" "prd_to_dev" {
  name                      = "peer-prd-to-dev"
  resource_group_name       = azurerm_resource_group.network.name
  virtual_network_name      = azurerm_virtual_network.vnet.name
...
resource "azurerm_virtual_network_peering" "dev_to_prd" {
...
resource "azurerm_private_dns_zone_virtual_network_link" "storage_blob_prd" {
...
# ── Private Endpoint: pe-eip-storage-prd ──────────────────────────────────────
